...
function track_command_install_complete {
  track_event INSTALL "\"command\":\"$1\", \"method\":\"$2\",\"step\":\"$3\""
}
...
function check_exec() {
  ...
  command -v $1 >/dev/null 2>&1  || { 
  		echo "[Info] I require $OUTPUT but it's not installed. $HELP." >&2
		EXIT_CODE=1 
	}
  if [[ $EXIT_CODE == "1" ]]; then
	  track_event CHECK_EXEC_NOT_INSTALLED "\"command\":\"$1\""
  else
	  track_event CHECK_EXEC_INSTALLED "\"command\":\"$1\""
  fi
  echo $EXIT_CODE
}
...
function do_install_python3_with_brew {
  ...
  track_event CHECK_EXEC_INSTALLED "\"command\":\"python3\""
}
...
function do_install_pip {
  ...
  track_event CHECK_EXEC_INSTALLED "\"command\":\"pip\""
}
...
function do_install_or_update_jasy {
  ...
  track_event CHECK_EXEC_INSTALLED "\"command\":\"jasy\""
}
...
function check_python3_and_install {
  ...
  if [[ $result == "1" ]]; then
      track_event CHECK_EXEC_NOT_INSTALLED "\"command\":\"$executable\""
      do_install_python3_with_brew
    else
      track_event CHECK_EXEC_INSTALLED "\"command\":\"$executable\""
